...
async function getHorsePayToken(): Promise<string> {
  ...
  try {
    ...
    console.log("📤 Enviando requisição de autenticação para:", `${config.horsepay.apiUrl}/auth/token`)
    console.log("📦 Payload:", { client_key: clientKey.substring(0, 10) + "...", client_secret: "***" })

    const authResponse = await fetch(`${config.horsepay.apiUrl}/auth/token`, {
      method: "POST",
      headers: {
